    ...
    <div class="inline-flex flex-row flex-wrap gap-(--space-xs)">
      <div v-for="member in members" :key="member.login">
        <a :href="member.html_url" target="_blank">
          <img :src="member.avatar_url" :class="`w-[${itemHeight}] h-[${itemHeight}] rounded-[${itemHeight}] transition-transform duration-200 ease-[cubic-bezier(0.175,0.885,0.32,2)] hover:scale-150`" >
        </a>
      </div>
    </div>
  </div>
